...
return [
    ...
    // เพิ่มเมื่อ 2026-08-30 — ข้อมูลจากสำนักงานพัฒนาสังคมและความมั่นคงของมนุษย์จังหวัด (พมจ.) ไม่ใช่
    // ข้อมูลรายสถานศึกษาเหมือนฟอร์มอื่นเลย ไม่มีคอลัมน์รหัสสถานศึกษาทั้ง 2 ชีท
    '16_pmj' => [
        'form_label'     => 'ตารางที่ 16 ข้อมูล พมจ.',
        'source_file'    => '16_ข้อมูล_พมจ_V2.xlsx',
        'sheets' => [
            [
                // ตารางที่ 16.1 จำนวนผู้พิการช่วงอายุ 6-11 ปี แยกตามความพิการและเพศ รายตำบล — ไม่มี
                // รหัสสถานศึกษาและไม่มีชื่อหน่วยงานเลย เป็นข้อมูลระดับพื้นที่ (ตำบล/อำเภอ) ล้วน ๆ —
                // ไม่มี "สังกัด/หน่วยงาน" ให้ใช้เป็นคีย์กันข้อมูลซ้ำตอนอัปโหลดไฟล์เดิมซ้ำ (เหมือนฟอร์ม
                // อื่นที่ไม่มีรหัสสถานศึกษา) จึงต้องเพิ่ม fallback ใหม่ใน
                // Importer::deleteExistingSubmission() ให้ใช้ตำบล+อำเภอแทนเป็นคีย์กันซ้ำ (ดู
                // src/Importer.php และ ai_note.md)
                'sheet_name'      => 'พมจ-16.1',
                'skip_rows'       => [1, 2],
                'header_rows'     => 5,
                'identity_cols'   => 2,
                'identity_fields' => ['tambon', 'amphoe'],
                'value_type'      => 'numeric', // จำนวนผู้พิการ แยกตามประเภทความพิการและเพศ
                'value_label'     => 'ประเภทความพิการ',
                'value_split_last' => 'เพศ',
            ],
            [
                // ตารางที่ 16.2 ข้อมูลสถานรับเลี้ยงเด็กเอกชน ผู้เรียนปฐมวัย และครู/ผู้สอน สังกัด พมจ.
                // — แม่แบบกรอกรายชื่อสถานรับเลี้ยงเด็ก (ศูนย์พัฒนาเด็กเล็ก) ที่ทราบอยู่แล้วไว้
                // ล่วงหน้าในคอลัมน์ "สังกัด" — เก็บเป็น school_name (ไม่ใช่ agency_name) ตามคำขอ
                // ผู้ใช้งาน (2026-08-30) แล้วให้ Importer ค้นหา school_code ให้อัตโนมัติโดยจับคู่ชื่อ
                // นี้กับ schools_master (ทำเนียบโรงเรียน) — ดู match_school_code_by_name และ
                // Importer::schoolCodesByName() เมื่อจับคู่ได้ agency_name/amphoe/tambon/department
                // จะถูกเติมอัตโนมัติผ่าน Reporting::schoolMasterOverrides() เหมือนฟอร์มอื่นทุกฟอร์ม
                // ที่มี school_code (ไม่ต้องทำอะไรเพิ่มที่นี่) — ถ้าจับคู่ไม่ได้ (ชื่อไม่ตรงทำเนียบ)
                // จะถูกตั้ง school_code_issue='missing' ให้ไปแก้ที่หน้า "รายการที่ต้องตรวจสอบ" เอง
                'sheet_name'      => 'พมจ-16.2',
                'skip_rows'       => [1, 2],
                'header_rows'     => 4,
                'identity_cols'   => 1,
                'identity_fields' => ['school_name'],
                'match_school_code_by_name' => true,
                'value_type'      => 'numeric', // จำนวนเด็ก/ผู้ดูแลเด็ก แยกตามช่วงอายุและเพศ
                'value_label'     => 'รายการ',
                'value_split_last' => 'เพศ',
            ],
        ],
